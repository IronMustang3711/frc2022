...
public class TalonUtil {
  ...
  private static final Set<ControlMode> CLOSED_LOOP_MODES
    = EnumSet.complementOf(EnumSet.of(ControlMode.Disabled,
  ...
  public static void check(ErrorCode err){
    if(ErrorCode.OK.equals(err)) return;
  ...
  static class ClosedLoopTelemetry implements Runnable {
    ...
    @Override
    public void run() {
      if (!CLOSED_LOOP_MODES.contains(talon.getControlMode())) return;
  ...
  static class MotionMagicTelemetry implements Runnable {
    ...
    @Override
    public void run() {
      if (!MOTION_PROFILE_MODES.contains(talon.getControlMode())) return;
